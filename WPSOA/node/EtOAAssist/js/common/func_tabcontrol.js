...
function AddWorkbookEvent() {
    ...
    wps.ApiEvent.AddApiEventListener("SheetActivate", OnSheetActivate);
    wps.ApiEvent.AddApiEventListener("WorkbookOpen", OnWorkbookOpen);
 ...
 *  提交成绩数据至数据库
 */
 function OnBtnUpData() {
    //检测是否有文档正在处理
    var etApp = wps.EtApplication();
    var l_doc = etApp.ActiveWorkbook;
    var l_sh = etApp.ActiveSheet;
    //var l_RowUsed = wps.PluginStorage.getItem("RowUsed");
    if (!l_doc) {
        alert("WPS当前没有可操作文档！");
        return;
    }
    ...
    xhr.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 201) {
            ...
            alert(JSON.parse(this.responseText).Content)
        }
    }
    xhr.open("POST","http://127.0.0.1:8001/updata",true);
    var dataStr = "["
    var ScoreCol = ""
    if (l_sh.Name == "成绩录入"){
        ScoreCol = "M";
        ...
        for (var i=5;i<50;i++){
            etApp.StatusBar = String(l_sh.Range(ScoreCol+String(i)).Value2)
            if(l_sh.Range(ScoreCol+String(i)).Value2!=""){
                dataStr+='{"id":'+l_sh.Range("A"+String(i)).Value2+',"score":'+String(l_sh.Range(ScoreCol+String(i)).Value2)+'},'
            }
        }
        xhr.send(JSON.stringify(dataStr.slice(0,-1)+"]"))
    }else if (l_sh.Name == "直录"){
        ScoreCol = "D";
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.setRequestHeader('Client', 'WPS');
        for (var i=5;i<50;i++){
            etApp.StatusBar = String(l_sh.Range(ScoreCol+String(i)).Value2)
            if(l_sh.Range(ScoreCol+String(i)).Value2!=""){
                //dataStr+='{"id":'+l_sh.Range("A"+String(i)).Value2+',"score":'+String(l_sh.Range(ScoreCol+String(i)).Value2)+'},'
                dataStr+='('+l_sh.Range("A"+String(i)).Value2+',"'+String(l_sh.Range("B"+String(i)).Value2)+'","'+String(l_sh.Range("B1").Value2)+'",'+String(l_sh.Range(ScoreCol+String(i)).Value2)+'),'
            }
        }
        xhr.send(JSON.stringify(dataStr.slice(0,-1)+"]"))
    } 
...
function OnGetEnabled(control) {
    ...
    //alert(l_Params)
    let OADocLandMode=Boolean(JSON.parse(l_Params).OADocLandMode);
    switch (eleId) {
